  ...
  render() {
    return html`
      <div @scroll="${this.handleScroll}" style="height: ${this.height}px; overflow: auto; position: relative; padding: 0;">
        ${this.chunk}
        <div style="opacity: 0; position: absolute; top 0; left: 0; width: 1px; height: ${this.total}px "></div>
      </div>
    `;
  }
  handleScroll(e) {
  ...
  renderChunk(first) {
    ...
    this.chunk = html`
      ${chunk.map(
        (artist, index) =>
          html`
            <album-art artist="${artist}" style="position: absolute; transform: translateY(${(first + index) * this.itemHeight}px);"></album-art>
          `
      )}
    `;
    this.requestUpdate();
